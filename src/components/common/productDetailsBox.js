...
export default function ProductDetailsBox(props) {
  ...
  return (
    ...
    <div className="col-md-6 col-lg-6">
      <div className={props.page === "details" ? "details-gallery" : "view-gallery"}>
        <div className={props.page === "details" ? "details-label-group" : "view-label-group"}>
          {/* <label className={props.page === "details" ? "details" : "view-label new">new</label> */}
          {data.discount === (undefined || null) ? null :
            <label className={props.page === "details" ? "details-label off" : "view-label off"}>{data?.discount}%</label>}
        </div>
        <Carousel>
          <div>
    ...
    <div className="col-md-6 col-lg-6">
      <div className={props.page === "details" ? "details-content" : "view-details"}>
        <h3 className={props.page === "details" ? "details-name" : "view-name"}>
          <Link to="" >{data.name}</Link>
        </h3>
        <div className={props.page === "details" ? "details-meta" : "view-meta"}>
          {/* <p>SKU:<span>1234567</span>
                        </p> */}
          <p>BRAND:<Link to="">{data.brand}</Link>
          </p>
        </div>
        <div className={props.page === "details" ? "details-rating" : "view-rating"}>
          <ProductRating rating={data.rating} review={data.review} />
        </div>
        <h3 className={props.page === "details" ? "details-price" : "view-price"}>
          <del>{data.mrp}</del>
          <span>{data.price}<small>/per</small>
          </span>
        </h3>
        <p className={props.page === "details" ? "details-desc" : "view-desc"}>{data.description}</p>
        ...
        <div className={props.page === "details" ? "details-list-group" : "view-list-group"}>
          <label className={props.page === "details" ? "details-list-title" : "view-list-title"}>variants:</label>
          <ul className={props.page === "details" ? "details-tag-list" : "view-tag-list"}>
            {(varList || []).map((item, index) => {
              return (
                <li key={index} >
                  <Link to="" onClick={() => (setVarID(item.product_verient_id), setdatachange(true))}>
                    {item.verient_name}
                  </Link>
                </li>
              )
        ...
        <div className={props.page === "details" ? "details-add-group" : "view-add-group"}>
